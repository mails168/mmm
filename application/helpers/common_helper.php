 ...
 * 2016.9.20 分类检索导航
*/
if (!function_exists('category_nav')){
    function category_nav(){  
        $CI = &get_instance();
        $CI->load->model('rush_model');
        $CI->load->library('memcache');

        $is_preview = intval(trim($CI->input->get('is_preview')));
        //定义 key 
        $key_cat_nav =  'category_nav_list';
        //获取key的值
        $category_list = $CI->memcache->get($key_cat_nav);

        if(!$is_preview && $category_list != FALSE ){
            return $category_list;
        }

        $category_list = $CI->rush_model->get_select_multi_type();
        $CI->memcache->save($key_cat_nav,$category_list, CACHE_TIME_NAVIGATION);
        return $category_list;
    }
}

if (!function_exists('hot_word')){
    function hot_word(){  
        $CI = &get_instance();
        $CI->load->model('hotword_model');
        $CI->load->library('memcache');

        $is_preview = intval(trim($CI->input->get('is_preview')));
        //定义 key 
        $key_cat_nav =  'hot_word_list';
        //获取key的值
        $word_list = $CI->memcache->get($key_cat_nav);

        if(!$is_preview && $word_list != FALSE ){
            return $word_list;
        }

        $word_list = $CI->hotword_model->all();
        $CI->memcache->save($key_cat_nav,$word_list, CACHE_TIME_NAVIGATION);
        return $word_list;
    }
}