...
class Wordpress_model extends CI_Model {
    ...
    public function get_relative_articles($cids2, $id){
        $cids = implode(',', $cids2);
        /*$sql = "SELECT distinct p.ID post_id,p.post_title,p.post_date,u.`display_name`, tr.term_taxonomy_id,pm2.`meta_value` 
         ...
         $where = '';
         if(!empty($cids2)){
	     $where .= " AND tr.term_taxonomy_id=$cids2[0]";
	 }
	$sql = "SELECT distinct p.ID post_id,p.post_title,p.post_date,p.comment_count, u.`display_name`, tr.term_taxonomy_id,pm.`meta_value` 
                FROM wp_posts p LEFT JOIN wp_term_relationships tr ON tr.`object_id` = p.ID 
                LEFT JOIN wp_postmeta pm ON p.`ID`=pm.`post_id` 
                
                LEFT JOIN wp_users u ON u.ID=p.`post_author` 
                WHERE p.post_type='post' AND p.post_status='publish' AND pm.`meta_key`='cover' AND p.ID != $id ".$where." 
                ORDER BY RAND()  
                limit 10";
        ...
        foreach ($rows as $row){
            $row->cover = $this->get_cover($row->meta_value);
        }
        return $rows;
    }
    public function get_article_detail($id){
        if (!is_numeric($id))
        return false;
        $sql = "SELECT post_id, post_author, comment_count,zan_count, post_title, `post_content`, post_date, `display_name`, GROUP_CONCAT(`taxonomy`,'=',`name`,'=',term_id SEPARATOR '&') tags, cover FROM (SELECT p.ID post_id,p.post_author,p.post_title,p.comment_count, p.zan_count,p.`post_content`,p.post_date,u.`display_name`,tx.`taxonomy`,t.`name`,t.term_id,pm.`meta_value` cover FROM wp_posts p LEFT JOIN wp_term_relationships tr ON tr.`object_id` = p.`ID` LEFT JOIN wp_users u ON u.ID=p.`post_author` LEFT JOIN wp_term_taxonomy tx USING(term_taxonomy_id) LEFT JOIN wp_terms t ON tr.`term_taxonomy_id`=t.`term_id` LEFT JOIN wp_postmeta pm ON p.`ID`=pm.`post_id` AND pm.`meta_key`='cover' WHERE p.`ID`=$id AND p.post_type='post' AND
...
//        print_r($arr);
    ...
    public function comments_list($id, $page=1){
        ...
        $sql = "SELECT a.comment_ID,a.comment_author,a.`comment_content`,b.comment_content parent_content,a.comment_date,a.yyw_user_id,b.yyw_user_id parent_id "
                . "FROM wp_comments a LEFT JOIN wp_comments b ON b.`comment_ID`=a.`comment_parent` "
                . "WHERE a.comment_post_ID=$id AND a.comment_approved = 1 $where "
                . "ORDER BY a.comment_ID DESC LIMIT ".$start.", ".$page_size;
...
>>>>>>> .r668
    ...
    public function get_comment($id){
        ...
        $res = $this->db_wp->query($sql)->row_array();
        
        $sql = "SELECT user_id,user_name,user_advar FROM ty_user_info WHERE user_id IN (".$res['yyw_user_id'].")";
        $res2 = $this->db->query($sql)->row_array();
        $res['user_advar'] = $res2['user_advar'];
        $res['comment_author'] = $res2['user_name'];
        return array('0' => $res);
    ...
    public function search_article($kw, $page = 1){
        ...
        $sql = "SELECT p.ID id,p.post_title title,p.post_date,u.`display_name` author,pm.meta_value cover "
                . "FROM wp_posts p "
                . "LEFT JOIN wp_postmeta pm ON p.`ID`=pm.`post_id` AND pm.`meta_key`='cover' "
                . "LEFT JOIN wp_users u ON u.ID=p.`post_author` "
                . "WHERE p.post_type='post' AND p.post_status='publish' AND p.post_title like '%$kw%'";
        //echo $sql;
        $rows = $this->db_wp->query($sql)->result_array();
        //tag
        $sql = "SELECT p.ID id,p.post_title title,p.post_date,u.`display_name` author,pm.meta_value cover "
                . "FROM wp_posts p LEFT JOIN wp_postmeta pm ON p.`ID`=pm.`post_id` "
                . "AND pm.`meta_key`='cover' LEFT JOIN wp_users u ON u.ID=p.`post_author` "
                . "LEFT JOIN wp_term_relationships tr ON tr.`object_id` = p.`ID` "
                . "LEFT JOIN wp_terms t ON tr.`term_taxonomy_id`=t.`term_id` "
                . "WHERE p.post_type='post' AND p.post_status='publish' AND t.`name`='$kw'";
	...
	//if (preg_match_all( "/<p(.*?)>.*vid=(.*)&amp;width=.*<\/p>/iUs", $content, $matches)) {
	if (preg_match_all( "/<iframe.*vid=([0-9a-zA-Z]*)&amp;width.*><\/iframe>/iUs", $content, $matches)){
	...
	<iframe allowfullscreen="" class="video_iframe" data-src="https://v.qq.com/iframe/preview.html?vid=${video_id}&amp;width=100%&amp;height=100%&amp;auto=0" frameborder="0"  scrolling="no" src="http://v.qq.com/iframe/player.html?vid=${video_id}&amp;auto=0" style="margin: 0px; padding: 0px; max-width: 100%; box-sizing: border-box !important; word-wrap: break-word !important; display: block; z-index: 1; width: 100% !important;  overflow: hidden;min-height:478px;" ></iframe></p>
EOD;
    ...
    // PC 分词搜索的视频
    public function get_search_video($filter){
        $where = "";
        if (!empty($filter['ids'])) {
            $where .= " AND wpt.`ID` in (".$filter['ids'].")";
        } else {
            $where .= " AND wpt.`post_title` LIKE '%".$filter['kw']."%'";
        }
        
        $sql = "SELECT wpt.`ID`,wpt.`post_title`, wpm.`meta_value` img "
                . "FROM wp_posts AS wpt "
                . "LEFT JOIN wp_postmeta AS wpm ON wpt.`ID` = wpm.`post_id` "
                . "WHERE wpm.`meta_key` = 'cover' ".$where." LIMIT 8 ";
        $query=$this->db_wp->query($sql);     
        $result=$query->result();
    ...
    public function get_videos_articles($id_arr){
        $sql = " SELECT p.ID, p.post_title, p.post_date, p.comment_count, u.display_name, GROUP_CONCAT(pm.meta_key, '=',  pm.`meta_value` SEPARATOR '&' ) meta, praise_num
                 FROM  wp_posts p 
                 INNER JOIN wp_term_relationships tr0 ON tr0.`object_id` = p.`ID`
                 LEFT JOIN wp_term_relationships tr  ON tr.`object_id` = p.`ID` 
                 LEFT JOIN wp_users u ON u.ID = p.`post_author` LEFT JOIN wp_postmeta pm ON pm.`post_id` = p.`ID` 
                 LEFT JOIN ((SELECT COUNT(id) praise_num, post_id  FROM wp_zan GROUP BY post_id)) pnum on p.ID = pnum.post_id
                 WHERE p.post_type = 'post' AND p.ID IN ($id_arr) AND p.post_status = 'publish' AND pm.meta_key IN ('views', 'cover','intro') 
                 GROUP BY p.ID";
        $res = $this->db_wp->query($sql);
        $articles = $res->result();
        foreach( $articles as &$article ){
            ...
            $cover = isset($article_meta['cover'])?$article_meta['cover']:null;
            $article->cover = $this->get_cover($cover);
            $article->intro = $article_meta['intro'];
        }
        return $articles;
    }
     ...
     * 视频列表
    */
    public function videos_list($filter){        
        $filter['page_size'] = isset( $filter['page_size'] ) ? $filter['page_size'] : M_LIST_PAGE_SIZE;
        $filter['record_count'] = 0;
        $filter['page_count'] = 0;
        
        $where = "";
        if (!empty($filter['kw'])) {
            $where .= " AND p.post_title LIKE '%".$filter['kw']."%'";
        }
        if (!empty($filter['tmid'])) {
            $where .= " AND tr.`term_taxonomy_id` = '".$filter['tmid']."'";
        }
        
        $sql = "SELECT COUNT(1) AS cnt 
                 FROM ( SELECT p.ID 
                 FROM  wp_posts p 
                 INNER JOIN wp_term_relationships tr0 ON tr0.`object_id` = p.`ID` AND tr0.term_taxonomy_id = ". POST_FORMAT_VIDEO ."  
                 LEFT JOIN wp_term_relationships tr ON tr.`object_id` = p.`ID` AND tr.term_taxonomy_id <> ". POST_FORMAT_VIDEO ." 
                 LEFT JOIN `wp_terms` t ON tr.`term_taxonomy_id` = t.`term_id` 
                 LEFT JOIN `wp_term_taxonomy` tt ON t.`term_id` = tt.`term_id`
                 LEFT JOIN wp_postmeta pm ON pm.`post_id` = p.`ID` 
                 WHERE p.post_type = 'post' AND p.post_status = 'publish' AND pm.meta_key IN ('cover') AND tt.`taxonomy` <> 'post_tag'".$where." GROUP BY p.ID) a";
	...
	$start = $filter['page'] < 1 ? 0:($filter['page'] - 1 ) * $filter['page_size']; //开始条数
        ...
        $sql = "SELECT p.ID, p.post_title, pm.meta_value AS meta 
                 FROM  wp_posts p 
                 INNER JOIN wp_term_relationships tr0 ON tr0.`object_id` = p.`ID` AND tr0.term_taxonomy_id = ". POST_FORMAT_VIDEO ." 
                 LEFT JOIN wp_term_relationships tr ON tr.`object_id` = p.`ID` AND tr.term_taxonomy_id <> ". POST_FORMAT_VIDEO ." 
                 LEFT JOIN `wp_terms` t ON tr.`term_taxonomy_id` = t.`term_id` 
                 LEFT JOIN `wp_term_taxonomy` tt ON t.`term_id` = tt.`term_id`
                 LEFT JOIN wp_postmeta pm ON pm.`post_id` = p.`ID` 
                 WHERE p.post_type = 'post' AND p.post_status = 'publish' AND pm.meta_key IN ('cover') AND tt.`taxonomy` <> 'post_tag'".$where." "
                . "GROUP BY p.ID ORDER BY p.post_date DESC LIMIT $start, ".$filter['page_size'];
        $res = $this->db_wp->query($sql);
        $articles = $res->result();
        foreach( $articles as &$article ){
            //$meta = $article->meta;
            ...
            //parse_str($meta, $article_meta);         
            //$cover = isset($article_meta['cover'])?$article_meta['cover']:null;
            $article->cover = $this->get_cover($article->meta);
            //$article->views = $article_meta['views'];
            //$article->intro = $article_meta['intro'];
        }
        //return $articles;
        return array('filter' => $filter, 'list' => $articles);
    }
    ...
    */
    public function article_list($filter){        
        ...
        if (!empty($filter['tmid'])) {
            $where .= " AND tr0.term_taxonomy_id = ".$filter['tmid']."";
        }

        $sql = "SELECT COUNT(1) AS cnt FROM (SELECT p.ID FROM  wp_posts p 
            INNER JOIN wp_term_relationships tr0 ON tr0.`object_id` = p.`ID` AND tr0.term_taxonomy_id <> ".POST_FORMAT_VIDEO."  
            LEFT JOIN wp_term_relationships tr ON tr.`object_id` = p.`ID` AND tr.term_taxonomy_id = ".POST_FORMAT_VIDEO."  
            LEFT JOIN `wp_term_taxonomy` tt ON tr0.term_taxonomy_id = tt.`term_id` 
            LEFT JOIN wp_postmeta pm ON pm.`post_id` = p.`ID` 
            WHERE p.post_type = 'post' AND p.post_status = 'publish' AND pm.meta_key IN ('cover', 'intro')".$where." AND tt.`taxonomy` <> 'post_tag' AND tr.`object_id` IS NULL 
            GROUP BY p.id) a";
	...
	$start = $filter['page'] < 1 ? 0:($filter['page'] - 1 ) * $filter['page_size']; //开始条数
        ...
        $sql = "SELECT  ID, post_title,GROUP_CONCAT(meta_key, '=',  `meta_value` SEPARATOR '&' ) meta FROM (SELECT p.ID, p.post_title,pm.meta_key, pm.`meta_value` 
            FROM  wp_posts p 
            INNER JOIN wp_term_relationships tr0 ON tr0.`object_id` = p.`ID` AND tr0.term_taxonomy_id <> ".POST_FORMAT_VIDEO." 
            LEFT JOIN wp_term_relationships tr ON tr.`object_id` = p.`ID` AND tr.term_taxonomy_id = ".POST_FORMAT_VIDEO." 
            LEFT JOIN `wp_term_taxonomy` tt ON tr0.term_taxonomy_id = tt.`term_id` 
            LEFT JOIN wp_postmeta pm ON pm.`post_id` = p.`ID` 
            WHERE p.post_type = 'post' AND p.post_status = 'publish' AND pm.meta_key IN ('cover', 'intro') 
            AND tt.`taxonomy` <> 'post_tag' ".$where." AND tr.`object_id` IS NULL GROUP BY p.id, pm.meta_key, pm.`meta_value`) a "
            . "GROUP BY id ORDER BY id DESC LIMIT $start, ".$filter['page_size'];
        $res = $this->db_wp->query($sql);
        $articles = $res->result();
        foreach( $articles as &$article ){
            $meta = $article->meta;
