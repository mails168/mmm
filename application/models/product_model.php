...
class Product_model extends CI_Model
{
	...
	{
		$sql = "SELECT p.*, p.is_best as is_zhanpin,f.flag_name,f.flag_url,b.brand_name,b.brand_info, b.brand_story, b.logo_160_73,b.brand_story, pp.display_name, pp.logo, pp.product_num, pp.provider_cooperation, p.product_desc_additional,pc1.category_name as category_name1,pc1.category_id as category_id1,pc2.category_name as category_name2,pc2.category_id as category_id2  
                ...
                LEFT JOIN ".$this->_db->dbprefix('product_category')." AS pc2 ON p.category_id = pc2.category_id 
                LEFT JOIN ".$this->_db->dbprefix('product_category')." AS pc1 ON pc2.parent_id = pc1.category_id 
	...
	*/
    ...
    public function get_course_list($page, $expire = false, $cid = 0){
        ...
        $sql = 'SELECT UNIX_TIMESTAMP(p.`package_name`) - UNIX_TIMESTAMP(NOW()) new_time, p.product_id, p.product_desc_additional, p.product_name, p.package_name, p.subhead, p.ps_num, p.shop_price, p.detail1,p.detail5, pg.`img_url` FROM '.$this->_db->dbprefix('product_sub').' sub INNER JOIN '.
        ...
        return array('course_list' => $course_list, 'page' => $page, 'pages' => $page_cnt);
    }
	...
	*/
    ...
    //PC 分词搜索的产品
    public function get_search_product($filter){
        $where = "";
        if (!empty($filter['ids'])) {
            $where .= " AND pif.product_id in (".$filter['ids'].") ";
        } else {
            $where .= " AND pif.product_name LIKE '%".$filter['kw']."%'";
        }
        
        if (isset($filter['genre_id']) && !empty($filter['genre_id'])) {
            $where .= " AND pif.genre_id = '".$filter['genre_id']."'";
        }
        
        if (isset($filter['is_best']) && !empty($filter['is_best'])) {
            $where .= " AND pif.is_best = '".$filter['is_best']."'";
        }
    	$sql = "SELECT pif.`product_id`,pif.`product_name`,pif.`market_price`,pif.`shop_price`,pgy.`img_url`,pze.`size_name`,pif.`subhead`,pif.`package_name` "
                ...
                . "LEFT JOIN ".$this->_db->dbprefix('product_gallery')." AS pgy ON pgy.`product_id` = pif.`product_id` AND psb.`color_id` = pgy.`color_id` AND pgy.`image_type` = 'default' "
                . "WHERE 1 ".$where." AND pif.source_id IN (0, '".SOURCE_ID_WEB."')  LIMIT 100";
    ...
    public function search_product($filter){
        $filter['page_size'] = isset( $filter['page_size'] ) ? $filter['page_size'] : LIST_PAGE_SIZE;
	    ...
	    $kw = $this->db_r->escape_like_str($filter['kw']);
	    
	    $kw_arrs = preg_split('[\s+]', $kw);
	    array_walk($kw_arrs, create_function('&$val', '$val = trim($val);')); 
	    foreach ($kw_arrs as $k_k => $k_v) {
	    	$where .= " AND (pif.product_name LIKE '%{$k_v}%' OR pif.product_sn LIKE '%{$k_v}%' OR pif.keywords LIKE '%{$k_v}%' OR b.brand_name LIKE '%{$k_v}%') ";
	    }
	}
        ...
        $sql = " SELECT COUNT(1) AS cnt FROM (SELECT pif.product_id "
                . "FROM ".$this->_db->dbprefix('product_info')." AS pif "
                . "LEFT JOIN ".$this->_db->dbprefix('product_sub')." AS psb ON psb.`product_id` = pif.`product_id`	"
                . "LEFT JOIN ".$this->_db->dbprefix('product_size')." AS pze ON pze.`size_id` = psb.`size_id` "
                . "LEFT JOIN " . $this->_db->dbprefix('product_brand') . " AS b ON pif.brand_id = b.brand_id "
                . "LEFT JOIN " . $this->_db->dbprefix('product_provider') . " AS pp ON pif.provider_id = pp.provider_id "
                . "LEFT JOIN ".$this->_db->dbprefix('product_gallery')." AS pgy ON pgy.`product_id` = pif.`product_id` AND psb.`color_id` = pgy.`color_id` AND pgy.`image_type` = 'default' "
                . "WHERE 1 ".$where." AND pif.source_id IN (0, '".SOURCE_ID_WEB."') GROUP BY pif.product_id) a";
        $query = $this->db_r->query($sql);
	...
	$start = $filter['page'] < 1 ? 0:($filter['page'] - 1 ) * $filter['page_size']; //开始条数
    	...
    	$sql = "SELECT pif.`product_id`, pif.product_desc_additional, pif.product_desc, pif.is_promote,pif.promote_start_date, pif.promote_end_date,pif.promote_price, pif.brand_name, pif.`product_name`,pif.`market_price`,pif.`shop_price`,pgy.`img_url`,pze.`size_name`,pif.`subhead`,pif.`package_name`, pif.`price_show` "
                ...
                . "LEFT JOIN " . $this->_db->dbprefix('product_provider') . " AS pp ON pif.provider_id = pp.provider_id "
                . "LEFT JOIN ".$this->_db->dbprefix('product_gallery')." AS pgy ON pgy.`product_id` = pif.`product_id` AND psb.`color_id` = pgy.`color_id` AND pgy.`image_type` = 'default' "
                . "WHERE 1 ".$where." AND pif.source_id IN (0, '".SOURCE_ID_WEB."') GROUP BY pif.product_id ORDER BY pif.product_id DESC LIMIT ".$start.", ".$filter['page_size'];
		
    	$query=$this->db_r->query($sql);		
    	
    	$result=$query->result();
        return array('filter' => $filter, 'list' => $result);
    }
    ...
    public function get_relation_product($category_id_arr){
        $sql = "SELECT p.*, pg.`img_url` FROM ty_product_info p 
            ...
            GROUP BY p.product_id LIMIT 20";
        $query =  $this->db_r->query($sql);
        $result = $query->result();
        return $result;
    }
    ...
    public function get_pc_index_brand_info($brand_ids)
    {	 
            $sql = "SELECT brand_id,brand_name,brand_logo FROM ty_product_brand WHERE brand_id IN ($brand_ids)";
    $query = $this->db_r->query($sql);
            return $query->result();
    }
    
    public function sub_array($sub_ids)	
    {
            $sql = " SELECT * FROM ".$this->_db->dbprefix('product_sub')." AS sub 
                             LEFT JOIN ".$this->_db->dbprefix('product_info')." AS pro ON pro.product_id = sub.product_id  
