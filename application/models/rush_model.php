...
function get_front_types($genre_id=1){
    ...
    public function brand_product_list($brand_id, $page){
        ...
        $sql = "SELECT p.product_id,p.product_name,p.market_price,p.shop_price,si.size_name,pg.`img_url`,p.price_show "
                ...
                . "LEFT JOIN ty_product_size si USING(size_id) "
                . "LEFT JOIN ty_product_gallery AS pg ON ps.`product_id`=pg.`product_id` AND ps.`color_id`=pg.`color_id` "
                . "LEFT JOIN ty_product_brand pb USING(brand_id) "
                . "WHERE pb.`brand_id`=$brand_id AND pg.image_type='default' AND p.`is_audit`=1 AND  ps.is_on_sale = 1 "
                . "GROUP BY p.`product_id` LIMIT $start, $page_size";
...
*/
    ...
    public function product_list2($filter , $slave = TRUE ,$is_preview = FALSE) {
        $this->adoEx = $slave ? $this->db_r : $this->db;
        $this->load->helper("product");
        $filter['page_size'] = is_int( LIST_PAGE_SIZE ) ? LIST_PAGE_SIZE : 30;
        $filter['record_count'] = 0;
        $filter['page_count'] = 0;
        $select = "SELECT sub.product_id,sub.gl_num as sub_gl_num,sub.consign_num,sub.wait_num,GROUP_CONCAT(sub.sub_id,'=', s.size_name SEPARATOR '&') AS sub_list,
                               if(SUM(GREATEST(sub.gl_num-sub.wait_num,0))+SUM(GREATEST(sub.consign_num,0)+IF(sub.consign_num=-2,1000,0))>0,1,0) AS gl_num,
                                      p.product_name,p.product_sn,p.shop_price,p.market_price,p.ps_num,p.pv_num,p.is_hot,p.is_new,p.is_offcode,p.is_best as is_zhanpin,
                                      p.is_promote,p.promote_price,p.promote_start_date,p.promote_end_date,p.is_gifts,p.product_desc_additional,p.price_show,p.product_desc,
                                      b.brand_name,g.img_url,pp.display_name,pp.provider_id ";
        ...
        $from = "FROM ty_product_sub AS sub
                ...
                LEFT JOIN " . $this->adoEx->dbprefix('product_gallery') . " AS g ON g.product_id=sub.product_id AND g.image_type='default'
                LEFT JOIN " . $this->adoEx->dbprefix('product_provider') . " AS pp ON p.provider_id = pp.provider_id 
                LEFT JOIN " . $this->adoEx->dbprefix('product_size') . " s ON sub.size_id = s.size_id ";
	...
	if($is_preview){
	   ...
	   $where = "WHERE p.genre_id = '".PRODUCT_TOOTH_TYPE."' AND p.source_id IN (0, '".SOURCE_ID_WEB."') AND p.price_show = 0 AND p.shop_price > 0 AND sub.is_on_sale=1 AND b.is_use = 1 AND pp.is_use = 1 "; 
        ...
        if (!empty($filter['type_id']) )//category_id
	    $where.=" AND (pt.type_id = '{$filter['type_id']}' OR pt.parent_id = '{$filter['type_id']}' OR pt.parent_id2 = '{$filter['type_id']}')";
	if (!empty($filter['cat']) )//category_id
	    $where.=" AND pt.parent_id IN (".$filter['cat'].")";
	if (!empty($filter['brand']))
	    $where.=" AND p.brand_id IN (".$filter['brand'].")";
	if (!empty($filter['kw'])) {
	    $kw = $this->adoEx->escape_like_str($filter['kw']);
	...
	if (!empty($filter['price'])) {
            ...
            $where.=" AND p.shop_price>='".intval($price[0])."' ";
	    if (!empty($price[1])) $where.=" AND p.shop_price<='".intval($price[1])."' ";
	}
    ...
    function get_select_multi_type($args = array() ,$slave = TRUE ){
        ...
        if (!empty($args) && $args['cat_type'] == 'c'){
            ...
            if (($cache_data = $this->cache->get($cache_key) ) === FALSE ) {
                ...
                foreach ($tmp_row as $i => $row2) {                            
                    if (!empty($row2['cat_content'])) {
                        $tmp_row2 = json_decode($row2['cat_content'], true);                        
        ...
        } elseif (!empty($args) && $args['cat_type'] == 'b') {
            $cache_key .= 'type_b_' . $args['brand'].'_'.$args['cat'];
            if (($cache_data = $this->cache->get($cache_key) ) === FALSE ) {
                $this->cache->delete($cache_key);
                $sql = "SELECT * FROM ty_product_brand WHERE brand_id IN (".$args['brand'].")";
                $tmp_row = $this->db_r->query($sql)->result_array();
                $row = array();
                foreach ($tmp_row as $i => $row2) {
                    if (!empty($row2['cat_content'])) {
                        $tmp_row2 = json_decode($row2['cat_content'], true);
                        foreach ($tmp_row2['cat'] as $id => $r){                          
                            if(isset($cat_arr) && in_array($id, $cat_arr)) {
                                $row['filter_type']['title'][$id] = $r['name'];
                                
                                if (!isset($row['filter_type']['display'])) $row['filter_type']['display'] = array();
                                if ((count($row['filter_type']['display']) < 3)) $row['filter_type']['display'][$id] = $r['name'];
                            } elseif (!isset($cat_arr)) {
                                $row['cat'][$id] = $r;
                            }
                        }
                    }
                    $row['filter_brand']['title'][] = $row2['brand_name'];
                    if ($i < 3) $row['filter_brand']['display'][] = $row2['brand_name'];
                }
                $cache_data = $row;
                $this->cache->save($cache_key, $row , CACHE_TIME_SELECT_TYPE);
            }
            ...
            if (($cache_data = $this->cache->get($cache_key) ) === FALSE ) {
                $this->cache->delete($cache_key);
                $sql = "SELECT * FROM ty_product_provider WHERE provider_id = 1";
                $row2 = $this->db_r->query($sql)->row_array();
                $row = array();
                if (!empty($row2['cat_content'])){
                    $row = json_decode($row2['cat_content'], true);
                }
                $cache_data = $row;
                $this->cache->save($cache_key, $row , CACHE_TIME_SELECT_TYPE);
            }
