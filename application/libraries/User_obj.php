...
class User_obj {
    ...
    public function login($mobile = '', $password = '', $remember = 0)
    {
		...
		//else $field = 'mobile';
        if( is_mobile_number($mobile) ) $field = 'mobile';
...
//	    }
    ...
    {
    	$this->CI->load->model('user_model');
    	if (empty($alipay_id)) return array();
...
*/
	...
	{
	    if(empty($send_type)) return false;
	    if(empty($arg['mobile'])) return false;
	...
	*/
    ...
    {
    	if(empty($send_type)) return FALSE;
		if(empty($arg['to_email'])) return FALSE;
