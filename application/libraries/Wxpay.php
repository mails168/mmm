<?php
	...
	*/
	...
	class WxPayConf_pub
	{
		...
		const APPID = 'wxd11be5ecb1367bcf';
		//受理商ID，身份标识
		const MCHID = '10029377';
		//商户支付密钥Key。审核通过后，在微信发送的邮件中查看
	//	const KEY = '88888888888888888888888888888888';
		
		const KEY = '6d05ab776fd92157d6833e2936d6f17c';
		//JSAPI接口中获取openid，审核后在公众平台开启开发模式后可查看
		const APPSECRET = '6d05ab776fd92157d6833e2936d6f17c';
		
		//=======【JSAPI路径设置】===================================
		//获取access_token过程中的跳转uri，通过跳转将code传入jsapi支付页面
		const JS_API_CALL_URL =  '/order/pay';
		
		//=======【证书路径设置】=====================================
		//证书路径,注意应该填写绝对路径
		const SSLCERT_PATH = 'wxpay_pem/cacert/apiclient_cert.pem';
		const SSLKEY_PATH = 'wxpay_pem/cacert/apiclient_key.pem';
		
		//=======【异步通知url设置】===================================
		//异步通知url，商户根据实际开发过程设定
		const NOTIFY_URL =  '/pay/wxpay_notify';
	...
	*/
	...
	class CI_Wxpay {
		...
		public function getWxpayParameters($order) {
		$viewData['out_trade_no_1'] = $order->track_sn;
		$viewData['total_fee'] = $order->pay_price;
		$viewData['goodsTitle'] = $order->track_sn; 	
	...
	//=========步骤1：网页授权获取用户openid============
	$CI=&get_instance();
	$weixin_config = $CI->config->item('weixin');
	//通过code获得openid
	if (!isset($_GET['code']))
	{
		//触发微信返回code码
				
			$url = $jsApi->createOauthUrlForCode($weixin_config['site_url'] . "/order/pay/$order->order_ids" . '?sn=' . $out_trade_no_1);
		Header("Location: $url"); 
	}else
	{
	...
	$unifiedOrder->setParameter("openid","$openid");//商品描述
		...
		$unifiedOrder->setParameter("body",$viewData['out_trade_no_1']);//商品描述
	//自定义订单号，此处仅作举例
	$timeStamp = time();
	//$out_trade_no = WxPayConf_pub::APPID."$timeStamp";
		$unifiedOrder->setParameter("out_trade_no",$viewData['out_trade_no_1']);//商户订单号 
	$unifiedOrder->setParameter("total_fee","$money");//总金额
	//$unifiedOrder->setParameter("total_fee","1");//总金额
	//$unifiedOrder->setParameter("notify_url", WxPayConf_pub::NOTIFY_URL);//通知地址 
		//$unifiedOrder->setParameter("notify_url", WxPayConf_pub::NOTIFY_URL);//通知地址 
			$unifiedOrder->setParameter("notify_url", $weixin_config['site_url'] . '/pay/wxpay_notify.html'); 
	...
	//var_export($unifiedOrder->parameters);exit;
	$prepay_id = $unifiedOrder->getPrepayId();
	//=========步骤3：使用jsapi调起支付============
	$jsApi->setPrepayId($prepay_id);

	$jsApiParameters = $jsApi->getParameters();	
		$viewData['jsApiParameters'] = $jsApiParameters;
		return $viewData;
		}
		...
		public function getJsApiParameters($order) {
			...
			if (!isset($_GET['code']))
			{
				...
				$url = $jsApi->createOauthUrlForCode($weixin_config['site_url'] . "/order/pay/" . $order->order_ids);
				Header("Location: $url"); 
			}else
			{
			...
			//echo "first $openid</br>";
			$unifiedOrder->setParameter("body",$order->track_sn);//商品描述
			//自定义订单号，此处仅作举例
			$timeStamp = time();
			//$out_trade_no = WxPayConf_pub::APPID."$timeStamp";
			$unifiedOrder->setParameter("out_trade_no", $order->track_sn);//商户订单号 
			$unifiedOrder->setParameter("total_fee","$money");//总金额
			//$unifiedOrder->setParameter("total_fee","1");//总金额
			//$unifiedOrder->setParameter("notify_url", WxPayConf_pub::NOTIFY_URL);//通知地址 
			$unifiedOrder->setParameter("notify_url", $weixin_config['site_url'] . '/pay/wxpay_notify.html');//通知地址 
			//	$unifiedOrder->setParameter("notify_url",'http://wwwt.yueyawang.com/payment/wxpay_notify.html'); 
			$unifiedOrder->setParameter("trade_type","JSAPI");//交易类型
			//非必填参数，商户可根据实际情况选填
			//$unifiedOrder->setParameter("sub_mch_id","XXXX");//子商户号  
			//$unifiedOrder->setParameter("device_info","XXXX");//设备号 
