...
class Cart extends CI_Controller {
	...
	//$provider_id_arr = array_keys($cart_summary['product_list']);
        ...
        */
        if (!empty($cart_summary['category_ids'])){
            $recomd_html = get_recommend_data(array('category_ids' => $cart_summary['category_ids']));
        } else {
            $recomd_html = get_recommend_data();
        }
    ...
    private function add_to_cart_2($sub_nums) {
            $this->load->model('order_model');
            $this->load->helper('product');
            ...
            $nums = '';
            $sub_ids = '';
            foreach ($sub_nums as $y => $v) {
                $nums += $v[1];
                $sub_ids .= $v[0].',';
            }
            
            if ($cart_summary['product_num'] + $nums > CART_SIZE){        
                $result['msg'] = '购物车已满，您的购物车最多可以保存 ' . CART_SIZE . ' 件商品。';
                $result['err'] = 1;
                return $result;
            }

            $sub = $this->product_model->sub_array(rtrim($sub_ids, ","));
            $update_batch = array();
            $insert_batch = array();

            foreach ($sub as $k => $val_sub) {
                if (empty($val_sub)) {        
                    $result['msg'] = '请选择颜色尺码';
                    $result['err'] = 1;
                    return $result;
                ...
                format_sub($val_sub);
                format_product($val_sub);

                if (!$type){
                    $type = $val_sub->is_promote ? 1 : 0;
                }

                $num = '';
                if($sub_nums[$val_sub->sub_id][0] == $val_sub->sub_id){
                    $num = $sub_nums[$val_sub->sub_id][1];
                };

                $update = array(
                    'user_id' => $this->user_id,
                    'session_id' => $cart_sn,
                    'sub_id' => $val_sub->sub_id,
                    'product_id' => $val_sub->product_id,
                    'color_id' => $val_sub->color_id,
                    'size_id' => $val_sub->size_id,
                    'shop_price' => $val_sub->shop_price,
                    'product_price' => $val_sub->product_price,
                    'product_num' => $num,
                    'update_date' => $this->time,
                    'discount_type' => $type, 
                    'shop_id' => $val_sub->shop_id
                );
                // 如果已有同样的商品，则不作操作，否则插入新值
                $old_cart = NULL;
                foreach ($cart_list as $c) {
                ...
                if ($old_cart) {
                    $update['rec_id'] = $old_cart->rec_id;
                    $update['product_num'] = $old_cart->product_num + $num;
                    $update_batch[] = $update; 
                } else {
                    $update['create_date'] = $this->time;
                    $insert_batch[] = $update; 
            ...
            if(!empty($update_batch))
                $this->cart_model->update_batch($update_batch, 'rec_id');
            if(!empty($insert_batch))
                $this->cart_model->insert_batch($insert_batch);

            // 刷新购物车
            $this->cart_model->refresh_cart($cart_sn);
            $this->db->trans_commit(); //结束事务啦啦啦
            $this->input->set_cookie('cart_num', $cart_summary['product_num'] + $nums, CART_SAVE_SECOND);
            $result = array('err' => 0, 'msg' => '');
            return $result; 
    }
    ...
    public function add_mutlit_subs_to_cart() {
        ...
        $result = $this->add_to_cart_2($sub_nums);
        print json_encode($result);
    }
    ...
    public function checkout($shop_id=0, $rec_ids='') {
        ...
        $checkout = get_checkout();  //当前session保存的结算信息
        ...
        $this->load->vars(array(
            ...
            'pay_list' => $pay_list,
            'default_pay_id' => PAY_ID_ALIPAY, //支付宝
            'to_pay_id' => PAY_ID_COD,  //货到付款
	...
	$this->load->model('user_model');
        ...
        */
        ...
        if ($genre_id == PRODUCT_COURSE_TYPE) {
            ...
            $this->load->view('cart/success_course1');
        } else {
            $this->load->view('cart/success3');
        }
    }
    ...
    public function load_address_form() {
        ...
        $html = $this->load->view('cart/address_block_new', array(
    ...
    public function submit_address_form() {
        ...
        $html = $this->load->view('cart/address_list_new', array(
    ...
    public function get_product_sizes($product_id){
        ...
        $data = array();
        //$rec_id = intval($this->input->get_post('rec_id'));
        $goods = $this->product_model->get_search_product(array('ids' => $product_id));
        if (!empty($goods)){
            $goods = $goods[0];
        } else {
            sys_msg('记录不存在', 1);
        }
        format_product($goods);
        ...
        if ($this->user_id) {
            //判断收藏的商品是否已收藏
            $col=$this->product_model->filter_collect(array('product_id'=>$product_id,'product_type'=>0,'user_id'=>$this->user_id));	
            if (!empty($col)) {
                $is_collected = true;
            }
        }
        ...
        $html = $this->load->view('category/product_size', $data, true);
        die(json_encode(array('err' => 0, 'msg' => '', 'html' => $html)));
    }
