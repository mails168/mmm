...
//			'css'		=> array('css/plist.css'),
	...
	*/
	...
	{	
        ...
        $product_register = $this->product_model->get_product_additional($product_id);
        ...
        $user_name = $this->session->userdata('user_name') ? $this->session->userdata('user_name') : '';
        $rank_name = $this->session->userdata('rank_name') ? $this->session->userdata('rank_name') : '';
        $mobile = $this->session->userdata('mobile') ? $this->session->userdata('mobile') : '';
        $p->detail1 = adjust_path($p->detail1);
        $p->detail2 = adjust_path($p->detail2);
        $p->product_desc = adjust_path($p->product_desc);
        ...
        $liuyan_list = $this->liuyan_model->liuyan_list($param);
         ...
         $cookiekey = PRODUCT_HISTORY_KEY;
         $recentBrowseProd = $this->input->cookie($cookiekey);
         $recentProKey = '';
         if($recentBrowseProd){
             $recentProArr = explode(',', $recentBrowseProd);
             $recentProKey = array_search($product_id, $recentProArr);
         } 
         else $recentProArr = array();
        
         if($recentProKey) unset($recentProArr[$recentProKey]);
         $rec_filter = array();
         $rec_filter['category_ids'] = array($p->category_id);
         $recommend_html = get_recommend_data($rec_filter);
         if (count($recentProArr) > 5) array_shift($recentProArr);
         $recentProArr[] = $product_id;
        
         $recentBrowseProd = implode(',', $recentProArr);

         $this->input->set_cookie($cookiekey, $recentBrowseProd, CACHE_TIME_PC_INDEX_PRODUCT_INFO);
...
//			'css'		=> array('css/plist.css'),
	...
	*/
	...
	public function ajax_get_history() {
		$history_html = get_history();
		echo $history_html;
	}

	public function clean_history() {
		$this->input->set_cookie(PRODUCT_HISTORY_KEY, '', time() - 3600);				
	}
}
