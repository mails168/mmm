...
class User extends CI_Controller
{
    ...
    $baby_list = array();
		$user_modify['email'] = trim($this->input->post('email'));
                ...
                if (empty($user_modify['email'])) unset($user_modify['email']);
                if (empty($user_modify['mobile'])) unset($user_modify['mobile']);
...
/*		$baby_birthdayYear = intval(trim($this->input->post('baby_birthdayYear')));
        ...
        $j = 1;
        if(empty($baby_list)) $baby_list = array();
    ...
    public function edit(){
        ...
        if ($res){
            ...
            echo json_encode(array('res'=>true,'msg'=>'恭喜您'.$point_amount.'积分已经到您的个人账户!', 'share_url' =>$share_url));
            else
...
// 如果短信已经发出10分钟内不再重发
	...
	if ($mobile == $sending_mobile_phone && (time()-$sending_mobile_code_time)<600 ){ // 10 mins
		$msg ='';
	...
	public function exchange_voucher(){
		if(!$this->user_id) goto_login ('user/exchange_voucher');
...
//		}
	...
	public function proc_exchange_voucher(){
		...
		if(!$user) sys_msg('兑换前请先登录',1);
		$release=$this->voucher_model->lock_release($release_id);
		if(!$release||$release->release_status!=1) sys_msg('该兑换活动当前尚未开始,敬请期待。',1);
		$campaign=$this->voucher_model->filter_campaign(array('campaign_id'=>$release->campaign_id));
		if($campaign->campaign_type!='ex' || $campaign->campaign_status!=1 || $campaign->start_date>$this->time || $campaign->end_date<$this->time)  sys_msg('该兑换活动当前尚未开始,敬请期待。',1);
		if($release->worth>$user->pay_points) sys_msg('您当前的积分不足',1);
	...
	public function qq_callback()
	{
	    ...
	    if (!empty($user->openid ))
	    {
		    ...
		    if (empty($nickname)) $nickname = 'qq用户';
	        ...
	        if(substr($back_url,0,4)!=='http') $back_url=base_url($back_url);
	        $this->session->unset_userdata('back_url');
	...
	public function xinlang_callback()
	{
		...
		if (!empty($uid ))
		{
		    ...
		    if (empty($nickname)) $nickname = '微博用户';
	...
	public function fc_callback() {
	    ...
	    if (empty($fc_user)) {//查看是否已经注册，no,新增（u_id@f-club.cn）；yes,查看是否完善信息,执行下一步
	    ...
	    if ($fc_user->password == '******' && $fc_user->email == "ufc_$fc_user->union_fclub@m.com") {//未完善，登录，提示完善资料
...
//	}
	...
	public function alipay_callback()
	{
		...
		if($verify_result) {//验证成功
		    ...
		    if (empty($nickname)) $nickname = '支付宝用户';
    ...
    function do_apply_return($order_id,$apply_id=0){
        ...
        if(!file_exists($img_path)) mkdir($img_path,0777);
        $img_path.='/'.date('m');
        if(!file_exists($img_path)) mkdir($img_path,0777);
        $img_path.='/'.date('d');
        if(!file_exists($img_path)) mkdir($img_path,0777);
