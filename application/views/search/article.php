    ...
    <div class="ui-page clearfix">
        <div class="ui-page-wrap">                        
            <b class="ui-page-num">                            
                ...
                <?php } else { ?>
                <a class="ui-page-prev" href="javascript:filter_product('<?=$kw?>', <?=$filter['page']-1?>, '<?=$target?>', 3, <?=$tmid?>);">&lt;&lt;上一页</a>
                <?php } ?>                         
                ...
                <?php foreach ($arr_pagelist_num as $arr_list_num): ?>
                    ...
                    <?php } elseif ($arr_list_num != "...") { ?>
                        <a href="javascript:filter_product('<?=$kw?>', <?=$arr_list_num?>, '<?=$target?>', 3, <?=$tmid?>);"><?=$arr_list_num ?></a>
                    <?php } ?>
                <?php endforeach; ?>
                ...
                <?php } else { ?>
                <a class="ui-page-next" href="javascript:filter_product('<?=$kw?>', <?=$filter['page']+1?>, '<?=$target?>', 3, <?=$tmid?>);">下一页&gt;&gt;</a>
                <?php } ?>
            </b>                        
            <b class="ui-page-skip">
                    共<?=$filter['page_count']?>页，到第
                    <input type="text" class="ui-page-skipTo" size="3" name="page" value="<?=$filter['page']?>" onkeydown="javascript:if(event.keyCode==13){page_jump('<?=$kw?>', this, '<?=$target?>', 3, <?=$tmid?>);return false;}"/>页
                    <button type="button" class="ui-btn-s" onclick="page_jump('<?=$kw?>', this, '<?=$target?>', 3, <?=$tmid?>)">确定</button>
            </b>                       
